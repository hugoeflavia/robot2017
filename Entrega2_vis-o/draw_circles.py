...
while (True):
    ...
    if circles != None:
        ...
        for j in range(len(circles[0, :])):
            i =  circles[0,j]
            #anterior = circles[0, j-1]
            if not j+1>=len(circles[0, :]):
                posterior = circles[0, j + 1]
            # draw the outer circle
            # cv2.circle(img, center, radius, color[, thickness[, lineType[, shift]]])
            cv2.circle(bordas_color, (i[0], i[1]), i[2], (0, 255, 0), 2)
            # draw the center of the circle
            cv2.circle(bordas_color, (i[0], i[1]), 2, (0, 0, 255), 3)
            distancia = 4000/(i[2]*2)
            if(len(circles[0, :]) == 3):
                print(abs(circles[0, 0][0] -circles[0, 2][0] ))
                if(abs(int(circles[0, 0][0]) -int(circles[0, 2][0]) ) > abs(int(circles[0, 0][1]) - int(circles[0, 2][1]))):
                    direcao = "horizontal"
                else:
                    direcao = "vertical"
    ...
    cv2.putText(bordas_color, '{0} - {1}'.format(distancia,direcao), (0, 50), font, 2, (255, 255, 255), 2, cv2.CV_AA)
